...
//        $.getScript(("https:" == document.location.protocol ? "https://" : "http://") + "malsup.github.io/jquery.form.js");
    	$.getScript("resources/jquery/jquery.form.js");
    }
    var feature = {};
    ...
    $.fn.uploadFile = function (options) {
        ...
        var s = $.extend({
            ...
            totalSizeErrorStr: "is not allowed. Allowed Total Max size: ",
	   ...
	   this.startUpload = function () {
		...
		function validateTotalSize( files ){
			...
			if ( s.maxTotalFileSize != -1 && totalSize > s.maxTotalFileSize ) {
				return true;
			} else {
				return false;
			}
        ...
        function serializeAndUploadFiles(s, obj, files) {
        	if( ! validateTotalSize(files) ) {
        		for(var i = 0; i < files.length; i++) {
                    ...
                    if(s.maxFileSize != -1 && files[i].size > s.maxFileSize) {
                        if(s.showError) $("<div class='" + s.errorClass + "'><font color='red'><b>" + files[i].name + "</b> " + s.sizeErrorStr + getSizeStr(s.maxFileSize) + "</font></div>").appendTo(
                            obj.errorLog);
                        continue;
                    }
        		...
        		if(s.showError) $("<div class='" + s.errorClass + "'><font color='red'><b>Upload </b> " + s.totalSizeErrorStr + getSizeStr(s.maxTotalFileSize) + "</font></div>").appendTo(
                        obj.errorLog);
        	}
                    ...
                    if(s.showStatusAfterSuccess) {
                        ...
                        if(s.showDelete) {
                            ...
                            pd.del.click(function () {
		                        removeExistingFileName(obj, fileArray);
//                                pd.statusbar.hide().remove();
                                if(s.deleteCallback) s.deleteCallback.call(this, data, pd, fileArray);
                                obj.selectedFiles -= fileArray.length; //reduce selected File count
                                updateFileCounter(s, obj);
                    ...
                    if(s.showDownload) {
                        pd.download.show();
                        pd.download.click(function () {
                            if(s.downloadCallback) s.downloadCallback(data, pd, fileArray);
                        });
                    }
                    form.remove();
                },
